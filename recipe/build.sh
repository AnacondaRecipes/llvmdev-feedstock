...
if [[ $target_platform == osx-* ]]; then
  ...
  # LLVM > 14 does not properly set the build order on OSX when making the C bindings library. This results in an error
  # where libllvm-c.dylib needs to link against libllvm.dylib; however, libllvm.dylib is a symlink to
